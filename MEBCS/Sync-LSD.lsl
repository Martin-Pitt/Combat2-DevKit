...
-------------------------
https://github.com/Martin-Pitt/NexiiLSL/blob/main/scripts/sync-lsd.lsl
...
* The script will ignore updates from objects that are not verified via RSA keys (!! -- Modified from original sync-lsd.lsl script to match project)

*/

// Change per project
#define DATA_CHANNEL (COMBAT_CHANNEL - 11)
...
default
{
    ...
    listen(integer channel, string name, key identifier, string message)
    {
        ...
        {
            string signature = llJsonGetValue(message, ["signature"]);
